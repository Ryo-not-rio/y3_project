...
class TenKDownloader:
    ...
    def download(self, target='./data', reset_flag=False):
        ...
        for c in self.CIK:
            ...
            for each in result:
                ...
                filename = c + "_" + each[0]+str(each[each[1].rfind('.'):])[:-2] + ".htm"
                r = requests.get(each[1], headers=hdr)
    ...
    def _search_each(self, cik):
        ...
        search_page = requests.get(url, headers=hdr)
        ...
        for i in range(start_idx, end_idx+1):
            ...
            company_page = requests.get(sub_url, headers=hdr)
...
class TenKScraper:
    ...
    def scrape(self, input_path, output_path):
        ...
        if match is None:
            raise Exception(f'No matched sections: {self.section}, {self.next_section} found in {input_path}.')
        else:
            return rawText
...
if __name__=='__main__':
    ...
    downloader.download("D:\\PycharmProjects\\y3_project\\data")
    #
    scraper2 = TenKScraper('Item 7')
    scraper2.scrape('D:\\PycharmProjects\\y3_project\\data\\AAPL_20151028.htm', 'D:\\PycharmProjects\\y3_project\\data\\test2.txt')
